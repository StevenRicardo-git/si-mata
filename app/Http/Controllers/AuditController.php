            ...
            try {
                switch($auditableType) {
                    case 'Kontrak':
                        ...
                        if ($kontrak) {
                            $audit->penghuni_name = $kontrak->penghuni->nama ?? null;
                            $audit->unit_code = $kontrak->unit->kode_unit ?? null;
                        }
                        break;
            ...
            if ($first->audit_batch_id) {
                ...
                if ($desc) {
                    ...
                    if ($targetTypes->contains('Penghuni') && $group->contains(fn($a) => $a->event === 'created')) {
                        ...
                        $kontrakAudit = $group->first(fn($a) => class_basename($a->auditable_type) === 'Kontrak');
                    ...
                    } elseif ($targetTypes->contains('Kontrak') && $group->contains(fn($a) => $a->event === 'updated' && isset($a->new_values['status']) && $a->new_values['status'] === 'keluar')) {
                        $kontrakAudit = $group->first(fn($a) => class_basename($a->auditable_type) === 'Kontrak' && $a->event === 'updated');
                        if ($kontrakAudit) {
                            $nama = $kontrakAudit->penghuni_name ?? 'N/A';
                            $unit = $kontrakAudit->unit_code ?? 'N/A';
                            $first->batch_description = "Mengakhiri Kontrak: {$nama} | Unit: {$unit}";
                        }
                    } elseif ($targetTypes->contains('Blacklist') && $group->contains(fn($a) => $a->event === 'created')) {
                        $blacklistAudit = $group->first(fn($a) => class_basename($a->auditable_type) === 'Blacklist' && $a->event === 'created');
                         if ($blacklistAudit) {
